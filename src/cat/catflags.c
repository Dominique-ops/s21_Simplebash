...
void CatSetNonPrint(char* table[static 256]) {
  const char* table_1[] = {"^@", "^A", "^B", "^C", "^D",
                           "^E", "^F", "^G", "^H"};
  const char* table_2[] = {"^K", "^L", "^M", "^N",  "^O", "^P", "^Q",
                           "^R", "^S", "^T", "^U",  "^V", "^W", "^X",
                           "^Y", "^Z", "^[", "^\\", "^]", "^^", "^_"};
  const char* table_3[] = {
      ...
      "M-}",  "M-~",  "M-^?"};
  memcpy(table, table_1, sizeof table_1);
  memcpy(&table[11], table_2, sizeof table_2);
  memcpy(&table['~' + 1], table_3, sizeof table_3);
}